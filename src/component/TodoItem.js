...
import { colorChange, deleted, toggle } from "../redux/todo/action";

const TodoItem = ({ todo }) => {
  //const todoComplited=useSelector((state)=>state.todos);

  const dispatch = useDispatch();
  const { text, id, complited, color } = todo;
  ...
  const handleColorChange = (todoId, todoColor) => {
    dispatch(colorChange(todoId, todoColor));
  };
  ...
  return (
    <>
      <div className="flex justify-start items-center p-2 hover:bg-gray-100 hover:transition-all space-x-4 border-b border-gray-400/20 last:border-0">
        ...
        <div className={`select-none flex-1 ${complited && "line-through"}`}>
          {text}
        </div>
